...
1. `results`: a dataframe containing **variables** with selection frequency >= 1. If setting `bagFreq.sigMethod == "PST" or "PERT"`, the **P.value** and the **P.adjust** of each variable will be provided. If setting `bagFreq.sigMethod = "CEP"` or using the elbow point indicators, elbow point(s) will be marked with <strong>*</strong> and <strong>thresholds</strong> for each variable will be assigned.
